...
import 'package:handshake_flutter/src/utils/magic_hash.dart';
...
class HDWallet {
  ...
  String? get privKey {
    ...
    try {
      return HEX.encode(_bip32!.privateKey!.toList());
    } catch (_) {
      return null;
    }
  }
  ...
  HDWallet derivePath(String path) {
    if (_bip32 == null)
      return HDWallet(bip32: null, p2pkh: null, network: network);
  ...
  Uint8List sign(String message) {
    if (_bip32 == null) return Uint8List(0);
...
class Wallet {
  ...
  Uint8List sign(String message) {
    if (_keyPair == null) return Uint8List(0);
    Uint8List messageHash = magicHash(message, network);
