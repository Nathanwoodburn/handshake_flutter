...
class Transaction {
  ...
  bool hasWitnesses() {
    ...
    ins.forEach((txIn) {
      if (txIn.witness != null && txIn.witness!.isNotEmpty) hasWitness = true;
    });
    return hasWitness;
  }
  ...
  hashForWitnessV0(
    ...
    if ((hashType & SIGHASH_ANYONECANPAY) == 0) {
      ...
      ins.forEach((txIn) {
        writeSlice(txIn.hash ?? Uint8List(0));
    ...
    if ((hashType & 0x1f) != SIGHASH_SINGLE &&
      ...
      var txOutsSize = outs.fold(
          0,
          (int sum, output) =>
              sum + 8 + varSliceSize(output.script ?? Uint8List(0)));
      ...
      tbuffer = new Uint8List(8 + varSliceSize(output.script ?? Uint8List(0)));
      bytes = tbuffer.buffer.asByteData();
      toffset = 0;
    ...
    writeSlice(input.hash ?? Uint8List(0));
  ...
  hashForSignature(int inIndex, Uint8List prevOutScript, int hashType) {
    ...
    if ((hashType & 0x1f) == SIGHASH_NONE) {
      ...
      // https://github.com/handshake/handshake/blob/master/src/test/sighash_tests.cpp#L60
  ...
  _byteLength(_ALLOW_WITNESS) {
    ...
    return (hasWitness ? 10 : 8) +
        ...
        ins.fold(
            0,
            (sum, input) =>
                sum + 40 + varSliceSize(input.script ?? Uint8List(0))) +
        outs.fold(
            0,
            (sum, output) =>
                sum + 8 + varSliceSize(output.script ?? Uint8List(0))) +
  ...
  String toString() {
    ...
    return 'Transaction{version: $version, locktime: $locktime, ins: $ins, outs: $outs}';
  }
}
...
class Input {
  ...
  factory Input.clone(Input input) {
    return new Input(
      hash: input.hash != null ? input.hash : null,
      index: input.index,
      script: input.script != null ? input.script : null,
      sequence: input.sequence,
      value: input.value,
      prevOutScript: input.prevOutScript != null ? input.prevOutScript : null,
      pubkeys: input.pubkeys != null
          ? input.pubkeys?.map((pubkey) => Uint8List.fromList(pubkey)).toList()
          : null,
      signatures: input.signatures != null
          ? input.signatures
              ?.map((signature) => Uint8List.fromList(signature))
              .toList()
          : null,
    );
  }
  ...
  factory Output.clone(Output output) {
    return new Output(
      ...
      pubkeys: output.pubkeys != null
          ? output.pubkeys?.map((pubkey) => Uint8List.fromList(pubkey)).toList()
          : null,
...
bool _isP2PKHInput(script) {
  ...
  if (chunks == null) throw new ArgumentError('Could not decode chunks');
